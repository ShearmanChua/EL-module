...
def inferenceWrapper(cls):
    ...
    class Wrapper:
        ...
        def run_inference(self):
            ...
            if not error:
                ent_list = []
                for idx, mention in enumerate(self.wrap.mentions_to_link):
                    ent_dict = {}
                    print("Mention: ",
                          self.wrap.mentions_to_link[idx]["mention"])
                    print("Original sentence: ")
                    print(self.wrap.mentions_to_link[idx]["context_left"] + " " + self.wrap.mentions_to_link[idx]
                          ["mention"] + " " + self.wrap.mentions_to_link[idx]["context_right"])

                    if self.wrap.mentions_to_link[idx]["id"] in mentions_to_blink["ids"]:
                        index = mentions_to_blink["ids"].index(
                            self.wrap.mentions_to_link[idx]["id"])
                        ...
                        if scores[index][0] > -2.1:
                            ...
                            ent_dict['doc_id'] = self.wrap.mentions_to_link[idx]["doc_id"]
                            ent_dict['mention'] = self.wrap.mentions_to_link[idx]["mention"]
